/* BASS 指板记忆
 * 由六弦吉他版指板记忆站改造为四弦贝斯版：
 * - 指板替换为四弦标准贝斯（E A D G，从上到下为 1~4 弦：G、D、A、E）
...
const ROUND_STORAGE_KEY = "bass-fretboard-round-count";
const BEST_STORAGE_PREFIX = "bass-fretboard-best-";
const FRET_RANGE_STORAGE_KEY = "bass-fretboard-fret-range";
const STRING_STORAGE_KEY = "bass-fretboard-strings";
const MIN_FRET = 0;
const MAX_FRET = 24;
...
// 四弦贝斯标准定弦，从上到下为 1 弦(G) 至 4 弦(低音 E)
// name 直接由 pitch 推导（CHROMATIC[pitch]），保证「空弦音标识」与「实际音高」永不脱节、无法错乱
const STRINGS = [
  { name: CHROMATIC[7], pitch: 7, size: 2.6 }, // 1弦 G
  { name: CHROMATIC[2], pitch: 2, size: 3.2 }, // 2弦 D
  { name: CHROMATIC[9], pitch: 9, size: 3.8 }, // 3弦 A
  { name: CHROMATIC[4], pitch: 4, size: 4.6 }, // 4弦 低音 E
];
// 弦序号 1-4（索引 0 = 一弦G … 索引 3 = 四弦E）
...
function buildFretboard() {
  ...
  singleInlays.forEach((fret) => {
    ...
    marker.style.gridRow = "2 / 4"; // 位于第 2、3 弦之间（四弦贝斯的中央）
