/* chord-engine.js — 贝斯版和弦引擎：音乐理论数据与位置搜索算法（纯函数，无 DOM）
 *
 * 由吉他版 chord-engine.js 改造：
 *  - 调弦 TUNINGS 改为贝斯定弦（4 弦 / 5 弦 / Drop D / Low C / 半音降）
 *  - 弦序约定：索引 0 = 最低音弦（贝斯 4 弦 → E 弦；5 弦 → B 弦），索引 n-1 = 最高音弦（1 弦 G）
...
/* ---------- 调弦定义（半音值；索引 0=最低音弦 … 索引 n-1=最高音弦 1 弦 G） ---------- */
/* ---------- 调弦定义（半音值；索引 0=最低音弦 … 索引 n-1=最高音弦 1 弦 G） ----------
 * pitches  每根弦的空弦音名（C=0 半音）
 * octaves  每根弦对应的开弦八度编号（scientific pitch notation，0=最低）
 *          计算公式：openMidi = pitches[i] + 12 * (octaves[i] + 1)
 *          4 弦贝斯标准 E1-A1-D2-G2 → octaves = [1, 1, 2, 2]（+1 给 MIDI 公式）
 */
const TUNINGS = {
  standard: { cn: "标准四弦 E-A-D-G", pitches: [4, 9, 2, 7], octaves: [1, 1, 2, 2], desc: "E A D G，最常用的四弦贝斯定弦。" },
  fiveString: { cn: "五弦贝斯 B-E-A-D-G", pitches: [11, 4, 9, 2, 7], octaves: [0, 1, 1, 2, 2], desc: "B E A D G，最低多一根 B 弦，金属 / 摇滚常用。" },
  dropD: { cn: "Drop D", pitches: [2, 9, 2, 7], octaves: [1, 1, 2, 2], desc: "D A D G，4 弦（最粗）降全音，摇滚常用。" },
  lowC: { cn: "Low C 调弦", pitches: [0, 7, 2, 9], octaves: [1, 1, 2, 2], desc: "C G D A，整琴降大三度，金属 / 部分爵士常用。" },
  eb: { cn: "半音降调", pitches: [3, 8, 1, 6], octaves: [1, 1, 2, 2], desc: "E♭ A♭ D♭ G♭，整琴降半音。" },
};
