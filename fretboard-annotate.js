...
(function () {
  ...
  const SCALE_MIN = 0.4; // 最小用户缩放（画板缩小为 40%，仍整体可见）
  const SCALE_MAX = 4;   // 最大用户缩放（限制幅度，避免一次性放大到不可控）

  function clampScale(s) {
    return Math.max(SCALE_MIN, Math.min(SCALE_MAX, s));
  }

  // 渲染模型：stageInner 经 flex 居中，CSS transform-origin 为画板中心。
  // 本地点 p 相对舞台中心的屏幕偏移 = pan + (p - 画板中心) * scale，
  // 即画板中心相对舞台中心的偏移（像素）恰为 (panX, panY)。
  // 所有平移 / 缩放 / 取点换算均以此真实模型为准（此前旧模型与 CSS 渲染不一致，
  // 缩放非 1 时偏差随倍数放大，导致画面瞬间跳飞）。

  // 限制平移范围，保证画面不会完全脱离可视区域、始终可找回：
  // - 画板大于舞台（放大态）：视口始终完全落在画板内，任意时刻都有内容可见
  // - 画板小于舞台（缩小态）：画板整体保持可见，允许轻微越出视口边缘
  function clampPan() {
    ...
    const bw = boardW * s;
    const bh = boardH * s;
    const MARGIN = 48; // px，缩小态允许画板越出视口的最大距离
    const maxX = bw >= sw ? (bw - sw) / 2 : (sw - bw) / 2 + MARGIN;
    const maxY = bh >= sh ? (bh - sh) / 2 : (sh - bh) / 2 + MARGIN;
    panX = Math.max(-maxX, Math.min(maxX, panX));
    panY = Math.max(-maxY, Math.min(maxY, panY));
  }

  // 以屏幕坐标 (sx, sy) 为焦点缩放：保持该点在缩放前后屏幕位置不变。
  // 由 屏幕偏移 off = pan + (p - O)*s，焦点本地点在缩放前后不变：
  //   pan' = off - (off - pan) * (sNew / sOld)
  function zoomAround(sx, sy, ratio) {
    const sRect = stage.getBoundingClientRect();
    const ox = sx - (sRect.left + sRect.width / 2); // 焦点相对舞台中心
    const oy = sy - (sRect.top + sRect.height / 2);
    const sOld = baseScale * userScale;
    const sNew = sOld * ratio;
    if (sOld <= 0 || ratio <= 0) return;
    panX = ox - (ox - panX) * (sNew / sOld);
    panY = oy - (oy - panY) * (sNew / sOld);
  }

  // 工具：将屏幕坐标转为画板本地坐标（基于当前变换）
  function screenToLocal(sx, sy) {
    const sRect = stage.getBoundingClientRect();
    const ox = sx - (sRect.left + sRect.width / 2);
    const oy = sy - (sRect.top + sRect.height / 2);
    const s = baseScale * userScale;
    return {
      x: boardW / 2 + (ox - panX) / s,
      y: boardH / 2 + (oy - panY) / s,
    };
  }
  // 工具：将画板本地坐标转为屏幕坐标
  function localToScreen(lx, ly) {
    ...
    const s = baseScale * userScale;
    return {
      x: sRect.left + sRect.width / 2 + panX + (lx - boardW / 2) * s,
      y: sRect.top + sRect.height / 2 + panY + (ly - boardH / 2) * s,
    };
  }
  ...
  function onPointerMove(e) {
    ...
    if (pointers.size === 2) {
      ...
      // 双指整体平移：pan 即像素偏移，两指中心移动多少像素画板就跟进多少
      panX += curCenter.x - pinchCenter.x;
      panY += curCenter.y - pinchCenter.y;
  ...
  function onWheel(e) {
    e.preventDefault();
    // 平滑指数缩放：deltaY 线性映射到乘性倍率，单步倍率限幅，避免一步跳变过大
    const delta = e.deltaY * (e.deltaMode === 1 ? 16 : 1);
    let ratio = Math.exp(-delta * 0.0015);
    ratio = Math.max(0.8, Math.min(1.2, ratio));
    const target = clampScale(userScale * ratio);
    zoomAround(e.clientX, e.clientY, target / userScale);
    userScale = target;
    applyTransform();
  }
